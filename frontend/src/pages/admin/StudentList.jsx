    ...
    <DashboardLayout role="admin">
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-2xl font-black text-slate-900 tracking-tight">{t('student_directory')}</h2>
            <p className="text-sm text-slate-500 font-medium mt-1">{t('manage_monitor')}</p>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={handleExport} className="flex-1 sm:flex-none btn-modern-outline !py-2 !px-4 flex items-center justify-center gap-2">
              <Download className="w-4 h-4" /> <span className="text-xs font-bold">{t('export_csv')}</span>
            </button>
            <button onClick={() => { setSelectedStudent(null); setIsAddStudentOpen(true); }} className="flex-1 sm:flex-none btn-modern-primary !py-2 !px-4 flex items-center justify-center gap-2">
              <Plus className="w-4 h-4" /> <span className="text-xs font-bold">{t('add_new_student')}</span>
            </button>
          </div>
        </div>
        ...
        <ExportOptionsModal isOpen={isExportOpen} onClose={() => setIsExportOpen(false)} title={t('export_csv')} />
          ...
          <div className="p-5 border-b border-slate-100 flex flex-col gap-5">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5">
              <div className="relative flex-1 lg:max-w-md group">
                ...
                <input
                  type="text"
                  placeholder={t('search_students_detailed')}
                  value={searchQuery}
          ...
          {error && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="p-4 bg-red-50 border border-red-200 rounded-2xl flex items-start gap-3 mb-4 mx-5 mt-4">
              ...
              <div className="flex-1">
                <p className="text-xs font-bold text-red-900">{t('failed_load_students')}</p>
                <p className="text-xs text-red-700 mt-1">{error}</p>
              </div>
              <button onClick={fetchStudents} className="text-xs font-bold text-red-600 hover:text-red-700 underline">{t('retry')}</button>
            </motion.div>
          )}
            ...
            <table className="w-full text-left border-collapse min-w-[1000px]">
              <thead>
                <tr className="bg-slate-50/50 text-slate-400 text-[10px] font-black uppercase tracking-widest">
                  ...
                  <th className="px-6 py-4">{t('unique_id')}</th>
                  <th className="px-6 py-4">{t('student_info')}</th>
                  <th className="px-6 py-4">{t('school_class')}</th>
          ...
          <div className="p-5 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50/30">
            <div className="flex items-center gap-4 order-2 sm:order-1">
              <p className="text-xs text-slate-500 font-bold">
                {t('showing')} <span className="text-slate-900">{((pagination.page - 1) * pagination.limit) + 1}-{Math.min(pagination.page * pagination.limit, pagination.total)}</span> {t('of')}{" "}
                <span className="text-slate-900">{pagination.total}</span> {t('total_students')}
              </p>
              <select
